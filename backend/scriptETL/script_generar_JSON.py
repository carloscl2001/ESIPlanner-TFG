...
# Lista de códigos de asignaturas a excluir (global)
EXCLUDED_SUBJECT_CODES = {
    '21716051', '21717039', '21718046', 
    '21719047', '21714064', '21720050', 
    '21715077'
}
...
def process_pdf(pdf_path):
    ...
    # Extraemos el código del grado
    full_filename = os.path.splitext(os.path.basename(pdf_path))[0]
    degree_code = full_filename.split('.')[0]
    ...
    # Usamos la variable global para filtrar
    filtered_subject_codes = [
        code for code in sorted(subject_codes) 
        if code not in EXCLUDED_SUBJECT_CODES
    ]
    
    degree_data = {
        "code": degree_code,
        "name": degree_name,
        "subjects": [{"code": code} for code in filtered_subject_codes]
    }
...
def extract_data_from_pdfs():
    ...
    for pdf_file in tqdm(pdf_files, desc="PDFs procesados"):
        ...
        # Usamos solo el código del grado para el nombre del archivo JSON
...
def parse_ics_to_json(ics_content):
    ...
    for component in cal.walk():
        if component.name == "VEVENT":
            ...
            class_entry = next((c for c in courses[codigo]["groups"] if c["group_code"] == tipo_clase), None)
            if class_entry is None:
                class_entry = {
                    "group_code": tipo_clase,
                    "events": []
                }
                courses[codigo]["groups"].append(class_entry)
        ...
        try:
            ...
            for code, details in courses.items():
                if code not in all_courses:
                    ...
                    for class_entry in details["groups"]:
                        existing_class = next((c for c in all_courses[code]["groups"] if c["group_code"] == class_entry["group_code"]), None)
                        if existing_class:
                            for event in class_entry["events"]:
                                if event not in existing_class["events"]:
                                    existing_class["events"].append(event)
                        else:
                            all_courses[code]["groups"].append(class_entry)
